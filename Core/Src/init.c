...
void GPIO_Init(void){
    ...
    */
    ...
    // Настройка порта РF13 на вход со стяжкой (Pull-down)
    CLEAR_BIT(GPIOF->MODER, GPIO_MODER_MODER13_0);
    SET_BIT(GPIOF->PUPDR, GPIO_PUPDR_PUPD13_0);
    // CLEAR_BIT(GPIOF->PUPDR, GPIO_PUPDR_PUPD13_0);
    // Настройка порта РE9 на вход со стяжкой (Pull-down)
    CLEAR_BIT(GPIOE->MODER, GPIO_MODER_MODER9_0);
    SET_BIT(GPIOE->PUPDR, GPIO_PUPDR_PUPD9_1);
...
void ITR_init(void){
    ...
    MODIFY_REG(SYSCFG->EXTICR[2], SYSCFG_EXTICR3_EXTI9_Msk, SYSCFG_EXTICR3_EXTI9_PE);
    SET_BIT(EXTI->IMR, EXTI_IMR_MR9);
    SET_BIT(EXTI->RTSR, EXTI_RTSR_TR9);
    CLEAR_BIT(EXTI->FTSR, EXTI_FTSR_TR9);
